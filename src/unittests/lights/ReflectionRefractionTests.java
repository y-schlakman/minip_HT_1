...
public class ReflectionRefractionTests {
    ...
    public void twoSpheresOnMirrors() {
        ...
        Geometries triangles = new Geometries(
        ...
        Geometries spheresAndTriangles = new Geometries(
                spheres,
                triangles
        );
        scene.geometries.add(spheresAndTriangles);
        /*scene.geometries.add( //
                new Sphere(new Point3D(-950, -900, -1000), 400) //
                        .setEmission(new Color(0, 0, 100)) //
                        .setMaterial(new Material().setkD(0.25).setkS(0.25).setnShininess(20).setkT(0.5).setDiffuseRadius(30)),
        ...
        ImageWriter imageWriter = new ImageWriter("reflectionTwoSpheresMirroredWithGlossyAndAABB", 500, 500);
        Render render = new Render() //
                .setImageWriter(imageWriter) //
                .setCamera(camera) //
                .setRayTracer(new RayTracerBasic(scene).setDiffuseEnabled(true).setGlossyEnabled(true).setNumGlossyDiffuseRays(5))
                .setMultithreading(3);
     ...
     */
    @Test
    public void solarSystemTestWithoutAABB() {
        ...
        ImageWriter imageWriter = new ImageWriter("refractionSolarSystemWithoutGlossyAABB", 600, 600);
        Render render = new Render() //
                .setImageWriter(imageWriter) //
                .setCamera(camera) //
                .setRayTracer(new RayTracerBasic(scene))
    ...
    /**
     * Produces a model of the our solar system showcasing all lighting effects we have with AABB.
     */
    @Test
    public void solarSystemTestWithAABB() {
        ...
        //First adding the sun as it has special definitions.
        double planetScale = 1E2;
        Geometries sunAndStar = new Geometries(
                new Sphere(new Point3D(0, 0, 0), radii.get(0) * planetScale) //Sun
                        .setEmission(colors.get(0).reduce(3))
                        .setMaterial(new Material().setkD(0.3).setkS(0).setnShininess(0).setkT(0.95))
        );
        Geometries stars2And3 = new Geometries();
        Geometries stars4And5 = new Geometries();
        Geometries stars6And7 = new Geometries();
        Geometries stars8And9 = new Geometries();
        ...
        for (int i = 1; i < 10; ++i) {
            ...
            Geometry planet = new Sphere(pos, radii.get(i) * planetScale)
                    .setEmission(colors.get(i).reduce(1))
                    .setMaterial(new Material().setkD(2.767676544).setkS(0));
            if(i==1)
                sunAndStar.add(planet);
            else if(i==2 || i==3)
                stars2And3.add(planet);
            else if(i==4||i==5)
                stars4And5.add(planet);
            else if(i==6||i==7)
                stars6And7.add(planet);
            else
                stars8And9.add(planet);
            prevPos = pos;
        }
        ...
        //An arbitrary pentagon to meet the assignment requirements of three different types of bodies in the scene.
        Geometries weirdPoly = new Geometries(
                new Polygon(new Point3D(0, -roomHeight / 2, zPlane + roomDepth / 2), new Point3D(250, -roomHeight / 2, zPlane + roomDepth / 2),
                        new Point3D(250, -roomHeight / 2 + 250, zPlane + roomDepth / 2), new Point3D(125, -roomHeight / 2 + 375, zPlane + roomDepth / 2),
                        new Point3D(0, -roomHeight / 2 + 250, zPlane + roomDepth / 2))
                        .setEmission(new Color(java.awt.Color.BLUE).scale(1))
        ...
        Geometries starsSun123 = new Geometries(sunAndStar, stars2And3);
        Geometries stars4567 = new Geometries(stars4And5, stars6And7);
        Geometries stars = new Geometries(starsSun123, stars4567, stars8And9);
        Geometries floorAndWeirdPoly = new Geometries(floor, weirdPoly);
        scene.geometries.add(stars, floorAndWeirdPoly, wall);
        ...
        ImageWriter imageWriter = new ImageWriter("refractionSolarSystemWithGlossyWithAABB", 600, 600);
        Render render = new Render() //
                .setImageWriter(imageWriter) //
                .setCamera(camera) //
                .setRayTracer(new RayTracerBasic(scene))
